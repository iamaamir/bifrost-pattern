...
- `architecture.json`: `title`, `nodes`, `edges`, `flows`, and `recommendations`; `overview` is optional. When it helps newcomer understanding, add repository-specific `thesis`, meaningful systems (`id`, `label`, `purpose`), and `primaryFlow` (`title`, `summary`, ordered system-id `steps`). Treat overview-first, small system count, and hidden component detail as guidance, not taxonomy. Stronger repository-specific structure wins. Nodes may include `system` to enable selected-system detail, but it is optional. Every node: `id`, `label`, `kind`, `purpose`, non-empty `evidence`. Every secondary flow: `title`, `summary`, ordered node-id `steps`; every recommendation: `title`, `confidence`, `why`, `safeValidationCommand`;
