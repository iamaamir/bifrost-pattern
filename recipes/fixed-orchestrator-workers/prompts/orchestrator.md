...
3. Convert the preflight map into a tiny task pack: git SHA, graph snapshot id, relevant files, relevant symbols, known gotchas, test commands, artifact ids, and freshness warnings.
4. Select worker count and roles from effort estimate; retain direct outer work when that is lower-cost and sufficient.
