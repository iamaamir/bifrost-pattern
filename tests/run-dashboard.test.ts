...
test("projects redacted ledgers into configurable terminal dashboard", () => {
  ...
  writeFileSync(join(project, ".pi", "bifrost-debug.jsonl"), `${JSON.stringify({ pattern_run_id: "run-1", subagent_run_id: "worker-1", event: "total", model: "opencode/mini-mo", tier: "quick" })}\n`);
