...
Use verified facts and source-path evidence supplied by orchestrator. Do not invent architecture. Create clear `onboarding.md`, `CONTEXT.md`, and structured `architecture.json`; generate HTML/Markdown graph through supplied generator. Use overview-first, small system map, primary flow, and selected-system detail as guidance when they improve onboarding. Do not force them over a stronger repository-specific structure. Favor readable hierarchy, accessible native HTML, responsive layout, keyboard navigation, and actionable source evidence.
